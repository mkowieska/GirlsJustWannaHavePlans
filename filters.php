...
// Funkcja do sprawdzania, czy grupa istnieje w bazie danych
function checkGroupExistence($db, $groupName) {
    try {
        $stmt = $db->prepare("SELECT id FROM `Group` WHERE group_name = :groupName");
...
// Funkcja do sprawdzania, czy przedmiot istnieje w bazie danych
function checkSubjectExistence($db, $subjectName) {
    try {
        $stmt = $db->prepare("SELECT id FROM Subject WHERE name = :subjectName");
...
// Funkcja do sprawdzania, czy student istnieje w bazie danych na podstawie numeru albumu
function checkStudentExistence($db, $albumNumber) {
    try {
        $stmt = $db->prepare("SELECT id FROM Student WHERE album_number = :albumNumber");
        $stmt->bindParam(':albumNumber', $albumNumber, PDO::PARAM_STR);
        $stmt->execute();
